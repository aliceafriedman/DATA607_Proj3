...
Since the number of entries less than \$1000 is small (12), I will exclude them from the dataset rather than convert to thousands. 

```{r}
USSalaries <- USSalaries %>% filter(USSalaries < 9999999 & USSalaries > 0)
quantile(USSalaries[[1]], c(.1, .2, .3, .4, .5, .6, .7, .8, .9), na.rm = TRUE)
hist(USSalaries[[1]])
```
